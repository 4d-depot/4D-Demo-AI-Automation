...
Function analyzeWeatherRiskAsync($event : cs.EventEntity; $weatherData : Object; $eventLines : Collection; $callback : 4D.Function)
	...
	$system:=$system+"3) If forecast is WORSE than planned (e.g., planned sunny but rain forecast): propose adding protective services\n"
	$system:=$system+"4) If forecast is BETTER than planned (e.g., planned rain but sunny forecast): propose removing unnecessary services and suggest alternatives to maintain or increase revenue\n"
	$system:=$system+"5) If forecast matches the plan: acknowledge this and suggest monitoring or minor optimizations\n"
	$system:=$system+"6) For indoor/indifferent venues: only flag extreme conditions (storms, extreme heat/cold affecting guest arrival)\n"
	$system:=$system+"7) If the event is outdoor and an indoor alternative is available at the same venue, propose switching as one action\n\n"
